...
export async function timerController(root: HTMLElement) {
	...
	// html elements
	const timerCircleEl = required(
		root.querySelector<HTMLElement>(".timer-circle"),
		"timer-circle",
	);
	const timerTextEl = required(
		root.querySelector<HTMLElement>(".timer-text"),
		"timer-text",
	);
	const toggleBtnEl = required(
		root.querySelector<HTMLButtonElement>("#btn-timer-toggle"),
		"btn-timer-toggle",
	);
	const modifyBtnEl = required(
	...
	function updateUI(seconds: number, total: number) {
		...
		timerCircleEl.style.setProperty("--timer-progress", `${fraction * 360}deg`);
	}
